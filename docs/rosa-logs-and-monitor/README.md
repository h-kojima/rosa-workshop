...
- `Collect Applications logs`: アプリケーションログの収集。`openshift-*`,`kube-*`,`default`以外のプロジェクトにデプロイされるアプリケーションのログ(stdoutとstderrに出力されるログ)を収集します。
- `Collect Infrastructure logs`: インフラストラクチャーログの収集。ROSAクラスター作成時にデフォルトで作成される`openshift-*`,`kube-*`, `default`プロジェクトにある、インフラストラクチャー関連のログを収集します。
...
ROSAクラスターのPrometheusでは、Red HatのSREチームによって、ROSAクラスター全体のメトリクスデータが一定期間保存されていますが、利用者のプロジェクトに関するメトリクスデータの保存期間を変更できるようになっています。例えば、メトリクスデータを200GiBの永続ボリュームを利用して30日間保存するような設定例を考えます。
