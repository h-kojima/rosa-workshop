...
マイナーリリース間のアップグレードを実施する際に、ROSAクラスターで利用されるAWS IAMロールのアップグレードを先に実施しておく、などの前提条件が指定される場合があります。このときは、上記画像で提示される手順(「Learn more」と書かれている箇所)を実施すると、アップグレードを実施できるようになります。下記のナレッジベースがその1例となりますので、参考にしてください。
